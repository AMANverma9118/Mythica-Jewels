...
export default function Navbar() {
  ...
  const NavLink = ({ to, children }) => (
    <Link to={to} className="relative group transition-colors duration-300 hover:text-yellow-400">
      {children}
      <motion.span className="absolute bottom-0 left-0 w-full h-0.5 bg-yellow-400 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300 ease-out" style={{ originX: 0 }}/>
    </Link>
  );
    ...
    <>
      <header className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ease-in-out ${navClasses}`}>
        <div className="container mx-auto flex justify-between items-center p-4">
          <Link to="/" className="flex items-center space-x-2"><motion.svg className="h-8 w-8 text-yellow-400" viewBox="0 0 100 100" fill="currentColor" whileHover={{ scale: 1.1, rotate: 10 }}><path d="M50 0L61.8 18.2H81L66.6 30.9L72.4 50L50 38.2L27.6 50L33.4 30.9L19 18.2H38.2L50 0Z" /><path d="M50 55L19 75H81L50 55Z" /></motion.svg><span className="text-xl font-serif font-bold tracking-wider">Mythica Jewels</span></Link>
